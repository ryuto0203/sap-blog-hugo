...
<!-- 画像が必要な箇所：P2Pサイクル全体のフロー図（業務ステップとトランザクションコードを矢印でつないだ図） -->
<div class="image-placeholder" style="border: 2px dashed #ccc; padding: 40px; text-align: center; margin: 24px 0; background: #f9f9f9;">
  <p style="color: #999; font-size: 14px;">【画像が必要】P2Pサイクル全体フロー図<br>購買依頼(ME51N) → 発注(ME21N) → 入庫(MIGO) → 請求書照合(MIRO) → 支払(F110) の流れを示す図</p>
</div>
...
<!-- 画像が必要な箇所：MM01の画面キャプチャ（ビュー選択画面や基本データ入力画面） -->
<div class="image-placeholder" style="border: 2px dashed #ccc; padding: 40px; text-align: center; margin: 24px 0; background: #f9f9f9;">
  <p style="color: #999; font-size: 14px;">【画像が必要】MM01 資材マスタ作成画面のキャプチャ<br>ビュー選択画面と基本データ1タブの入力例を並べた画像が理想</p>
</div>
...
<!-- 画像が必要な箇所：ME51N画面キャプチャ（明細入力エリアの画像） -->
<div class="image-placeholder" style="border: 2px dashed #ccc; padding: 40px; text-align: center; margin: 24px 0; background: #f9f9f9;">
  <p style="color: #999; font-size: 14px;">【画像が必要】ME51N 購買依頼作成画面のキャプチャ<br>明細タブの品目・数量・納期入力フィールドがわかる画面</p>
</div>
...
<!-- 画像が必要な箇所：ME49の見積比較画面（複数仕入先の価格比較一覧） -->
<div class="image-placeholder" style="border: 2px dashed #ccc; padding: 40px; text-align: center; margin: 24px 0; background: #f9f9f9;">
  <p style="color: #999; font-size: 14px;">【画像が必要】ME49 見積比較画面<br>複数仕入先の価格・条件を横並びで比較している画面のキャプチャ</p>
</div>
...
<!-- 画像が必要な箇所：ME21Nの画面キャプチャ（ヘッダ部分と明細部分を合わせた全体画面） -->
<div class="image-placeholder" style="border: 2px dashed #ccc; padding: 40px; text-align: center; margin: 24px 0; background: #f9f9f9;">
  <p style="color: #999; font-size: 14px;">【画像が必要】ME21N 発注書作成画面のキャプチャ<br>ヘッダ（仕入先・日付）と明細（品目・数量・単価・納期）の入力画面</p>
</div>
...
<!-- 画像が必要な箇所：MIGOの入庫処理画面（発注番号参照後の明細入力画面） -->
<div class="image-placeholder" style="border: 2px dashed #ccc; padding: 40px; text-align: center; margin: 24px 0; background: #f9f9f9;">
  <p style="color: #999; font-size: 14px;">【画像が必要】MIGO 入庫処理画面のキャプチャ<br>発注番号を参照した後、明細に品目・数量・保管場所が表示されている状態の画面</p>
</div>
...
<!-- 画像が必要な箇所：MIROの画面キャプチャ（請求書ヘッダ入力と明細照合リストの全体画面） -->
<div class="image-placeholder" style="border: 2px dashed #ccc; padding: 40px; text-align: center; margin: 24px 0; background: #f9f9f9;">
  <p style="color: #999; font-size: 14px;">【画像が必要】MIRO 請求書照合画面のキャプチャ<br>上部に請求書ヘッダ情報、下部に発注明細との照合リストが表示されている画面</p>
</div>
...
<!-- 画像が必要な箇所：MMBEの在庫照会画面（プラント・保管場所ごとの在庫数が表示された一覧） -->
<div class="image-placeholder" style="border: 2px dashed #ccc; padding: 40px; text-align: center; margin: 24px 0; background: #f9f9f9;">
  <p style="color: #999; font-size: 14px;">【画像が必要】MMBE 在庫照会画面のキャプチャ<br>品目番号を入力後、プラント・保管場所別の在庫数量が階層表示されている画面</p>
</div>
...
<!-- 画像が必要な箇所：MD04の所要量照会画面（納期・数量・調達提案が時系列で表示されたリスト画面） -->
<div class="image-placeholder" style="border: 2px dashed #ccc; padding: 40px; text-align: center; margin: 24px 0; background: #f9f9f9;">
  <p style="color: #999; font-size: 14px;">【画像が必要】MD04 所要量照会画面のキャプチャ<br>品目の在庫推移・所要量・調達提案（購買依頼など）が時系列で並んだ画面</p>
</div>
...
<!-- 画像が必要な箇所：上記テーブルをビジュアル化した業務フロー図（担当部門の横線を交える「スイムレーン図」が理想） -->
<div class="image-placeholder" style="border: 2px dashed #ccc; padding: 40px; text-align: center; margin: 24px 0; background: #f9f9f9;">
  <p style="color: #999; font-size: 14px;">【画像が必要】P2Pサイクルのスイムレーン図<br>現場・購買部門・倉庫・経理の担当を横軸に、各ステップとトランザクションを縦の流れで示した図</p>
</div>
